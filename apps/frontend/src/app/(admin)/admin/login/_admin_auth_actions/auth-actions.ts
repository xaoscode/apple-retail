"use server";
import { z } from "zod";
import { logoutAdminService } from "./auth-service";
import { AdminAuthActionState } from "./initital-state";
...
export async function logoutAdminAction(prevState: AdminAuthActionState): Promise<AdminAuthActionState> {
	console.log("Hello From LogOut User Action");

	const responseData = await logoutAdminService();
	if (!responseData) {
		return {
			...prevState,
			apiErrors: "",
